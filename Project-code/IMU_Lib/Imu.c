...
Imu_Typedef Imu = {0.0f, 0.0f, 0.0f};
...
void Angle_data(){
	...
	if (sum_data == (uint8_t)imu_data[10] ){
		...
		Imu.Roll  = ((int16_t)(imu_data[3] << 8 |imu_data[2]))*const_angle_val;
		Imu.Pitch = ((int16_t)(imu_data[5] << 8 |imu_data[4]))*const_angle_val;
		Imu.Yaw   = ((int16_t)(imu_data[7] << 8 |imu_data[6]))*const_angle_val;
	}
